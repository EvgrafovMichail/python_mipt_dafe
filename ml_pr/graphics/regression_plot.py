...
def make_coords(abscissa, ordinates):
    ...
    mnk = np.linalg.lstsq(np.vstack(
        [abscissa, np.ones(len(abscissa))]).T, ordinates, rcond=None)[0]

    # sigma coeff

    sigma_y = ((1 / (size_of_sample - 2)) *
               np.sum((ordinates - mnk[0] * abscissa - mnk[1]) ** 2))

    sigma_a = ((sigma_y ** 2) / (size_of_sample *
                                 ((abscissa ** 2).mean() -
                                  (abscissa.mean()) ** 2))) ** 0.5

    sigma_b = ((sigma_y ** 2 * (abscissa ** 2).mean()) /
               (size_of_sample * ((abscissa ** 2).mean() -
                                  (abscissa.mean()) ** 2))) ** 0.5
...
def mnk_show(x_mnk, y1, y2, mnk, abscissa, ordinates, borders):
    ...
    if (borders):
        axis.plot(abscissa, y1, color="red", linestyle='-.',
                  label="q - coridor")
