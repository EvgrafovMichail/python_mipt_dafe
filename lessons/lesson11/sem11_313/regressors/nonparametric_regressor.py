...
class NonparametricRegressor(RegressorABC):
    ...
    def predict(self, abscissa: Union[Real, Iterable]) -> list:
        ...
        for abs_i in abscissa:
            ...
            predictions.append(self._compute_predictions(weights))
    ...
    def _compute_predictions(self, weight: list[float]) -> float:
        prediction = sum(
            ord_i * weight_i for ord_i, weight_i
            in zip(self._ordinates, weight)
        )
        
        return prediction / sum(weight)