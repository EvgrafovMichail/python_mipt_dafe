...
class KNN:
    ...
    def __init__(self, k_neighbours: int = 5, metric: str = "l1") -> None:
        if isinstance(k_neighbours, int) and Metrics(metric):
    ...
    def predict(self, points: np.ndarray) -> np.ndarray:
        ...
        dist_for_width_window = np.reshape(
            dist_for_width_window, (1, dist_for_width_window.shape[0])
        )
        dist_for_width_window = np.reshape(
            dist_for_width_window, (dist_for_width_window.shape[1], dist_for_width_window.shape[0])
        )
    ...
    def _distance(self, points_from: np.ndarray, points_to: np.ndarray):
        ...
        if self._metric == Metrics.Manhatten:
