...
def vizualize_classification(
    ...
    if len(colors) == 0:
        for label in np.unique(lables):
            axis.scatter(points[lables == label][:, 0], points[lables == label][:, 1], alpha=0.5)
    else:
        for i, label in enumerate(np.unique(lables)):
            color = colors[i % len(colors)]
            axis.scatter(
                points[lables == label][:, 0],
                points[lables == label][:, 1],
                color=color,
                alpha=0.5
            )
...
def vizualize_regression(
    ...
    if down_error is not None and up_error is not None:
        axis.plot(
            abscissa,
            down_error,
            label='error',
